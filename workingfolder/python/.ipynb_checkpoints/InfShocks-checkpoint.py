...
#
# The approach used by Francis et al.(2014) to identify technoogy shocks is shown to have better small sample properties. I also identify tech shocks in their approach. 
#
...
#
# where $\alpha = F e_j$, the $j$-th column of D, and $e_i$ is a $n \times 1$ vector that picks responses of $i$. 
#
...
def SVAR_MaxShare(rs,h):
    """
    inputs
    ------
    rs: result object of reduced VAR estimated using statsmodels.tsa.VAR
    h:  exogoneous forecast horizon
    ...
    dictionary:
      ...
      residuals: ndarray
               reduced var residuals 
      alpha_est:   ndarray
               alpha_est = F'e_j where F is otho matrix 
      eps_est: ndarray
    ...
    maxn = h+1
    ma_mats = ma_rep(var_coefs,maxn)   # D matrix: h+1 x k x k 
    
    ## Compute forecast errors for horizon h
    fe_mats = np.eye(k)   # k variables,thus k forecast errors  
    ...
    def fe_share(alpha,i):
        ...
        fe_contr= np.diag(fe_contr_mats)
        shares = np.divide(fe_contr,fe)        
        share_i = shares[i]  # the variable of forecast error, i.g. i=0 meaning labor productivity
    ...
    obj = lambda alpha: -fe_share(alpha,0)
    ...
    epsilon_est = np.dot(np.dot(alpha_est,
                                LA.inv(H)),
                         u.T)
...
tech_shock = SVAR_maxshare_rst['epsilon_est']
