...
model_sv_data = [
                ...
                ('horizon', int64),                   # forecast horizons 
                ('real_time',float64[:,:]),           # 2d array, 3 x T, real time permanent component of inf 
                ('history',float64[:,:]),             # 2d array, a longer history of permanent component of inf
                ('realized',float64[:])               # realized inflation 
]
...
class RationalExpectationSV:
    ...
    def SimForecasts(self):
        ...
        infoset = real_time 
        nowcast = infoset[0,:]  ## permanent income componenet 
        forecast = nowcast
        σs_now = infoset[1:2,:]  ## volatility now 
...
xx_real_time,xx_pc_real_time,vol_p_real_time,vol_t_real_time = ucsv_fake  

xx_realized = xx_real_time[1:-1]

xx_real_time= np.array([xx_pc_real_time,
                        vol_p_real_time,
                        vol_t_real_time]
                      )[:,0:-2]
...
class StickyExpectationSV:
    ...
    def SimForecasts(self,
        ...
        infoset = history 
        p_now, sigmas_p_now, sigmas_t_now= infoset[0,:],infoset[1,:],infoset[2,:]
