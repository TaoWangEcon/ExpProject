...
# + {"code_folding": [4, 26, 30, 43, 59, 110, 144, 182, 251, 313, 323, 352, 381, 391, 399, 421, 443, 469, 507, 545, 577, 606, 631, 645, 658, 664, 672, 684, 696, 711, 726, 738, 760, 814, 830, 854]}
...
################################
    ...
    def Forecaster(self):
        ...
        for i in range(n):
            Var_this_i = np.sum([lbd*(1-lbd)**tau*self.hstepvar(tau+horizon,sigma,rho) for tau in range(i + n_burn)])
        ...
        for i in range(n):
            nowcast_this_i = np.sum([lbd*(1-lbd)**tau*(rho**tau)*history[i+n_burn-tau] for tau in range(i+n_burn)])
        ...
        for i in range(n):
            Disg_this_i = np.sum([lbd*(1-lbd)**tau*(rho**(tau+horizon)*history[i+n_burn-tau] - forecast[i])**2 for tau in range(i+n_burn)])
        ...
        FEATV = np.empty(n)
        for i in range(n):
            FEVar[i] = np.sum([lbd*(1-lbd)**tau*
                            (np.sum([rho**(x+horizon)*FE[i-x] for x in range(tau) if x<=i])
                             - FE[i])**2 
    ...
    def ForecasterbySim(self,
        ...
        ## simulation
        np.random.seed(12345)
        update_or_not = np.random.binomial(1,
                                           p = lbd,
        ...
        for i in range(n_sim):
            for j in range(n_history):
                if np.any([x for x in range(j) if update_or_not[i,j-x] == True]):
                ...
                Vars_to_burn[i,j]= self.hstepvar((most_recent_when[i,j]+horizon),sigma,rho)
...
# + {"code_folding": []}
@jit
def PopulateSE(history,
               update_or_not,
               rho,
               sigma,
               horizon):
    n_sim = update_or_not.shape[0]
    n_history = update_or_not.shape[1]
    ...
    for i in range(n_sim):
        for j in range(n_history):
            if np.any([x for x in range(j) if update_or_not[i,j-x] == 1]):
                ...
                most_recent_when[i,j] = j
            nowcasts_to_burn[i,j] = history[j - most_recent_when[i,j]]*rho**most_recent_when[i,j]
            var_temp = np.sum([rho**(2*i)*sigma**2 for i in range(horizon+most_recent_when[i,j])])
            Vars_to_burn[i,j]= var_temp
    
    return nowcasts_to_burn,Vars_to_burn


# +
start_time = time.time()

xx = PopulateSE(history = AR1_simulator(0.95,0.1,200),
          update_or_not = np.random.binomial(1,0.3,size =[100,200]),
          rho = 0.9,
          sigma = 0.1,
          horizon = 1)
...
@jit
def bernl(n,p):
    xx = np.random.binomial(1,p,size=(n,n))
    return np.mean(xx)
