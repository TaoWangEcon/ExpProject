...
\begin{document}
	...
	This paper extends the abovementioned effort by estimating each theory parametrically accounting for cross-moment restrictions. This is a natural step forward from reduced-form tests of rationality widely seen in the literature. With comparable estimates of different theories, one can evaluate to what extent each theory fits the observed expectation data and inflation dynamics.  I evaluate each model of expectation in terms of its sensitivity of specification with respect to four criteria (1) what moments are used for estimation. (2) the specification of the underlying process of the inflation. (3)  if estimating the underlying process and expectations separately or jointly. The former basicaly assumes recovers the inflation process only based on inflation data while the later let the expectations to provide information used for inflation estimation as well. (4) if it accounts for households and forecasters' expectations equally well.  Broadly speaking, this framework is closer to the original spirit of rational expectation in the sense it focuses on the consistency of the model specification instead of the absolute rationality. 
...
\citet{coibion2012can} derive the impulse response of dispersion at time $t+k$ to a shock that realized at $t$. Disagreements increase after realization of the shock and gradually returns to its steady-state level.  
	...
	Different from the previous two theories, diagnostic expectation (\cite{bordalo2018diagnostic}) is developed in the literature to account for extrapolative expectation formation that features overreactions to news. Both SE and NI deviate from FIRE in terms of the information set available to the agents, while DE deviates from FIRE in terms of the process of the otherwise perfect information set. 		
