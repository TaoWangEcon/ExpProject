...
Another prediction from FIRE is that there is an unambigous reduction in uncertainty as one approaches to the date of realization, where the drop is exactly equal to the volatility of the realized shocks. Although quantitatively, it is hard to check this directly, at least qualitatively, one can look at the distribution of the uncertainy revision concentrate in the negative range. Figure \ref{RevisionHist} plots the average revision in mean forecasts and uncertainty from 1-year-ahead forecast in year $t-1$ to the current-year nowcast in year $t$. The more negative range in which the revision lies, the more ``rational'' of the forecast. Looking from the histograms, the uncertainty revision is indeed centerred around zero while slightly showing left-skewness. Overall, more forecasters fell more certainy for nowcast compared to his forecast one year before.  Unfortunately, since SCE does not provide the data structure that allows us to look at revision, we cannot make a comparison between two types of agents. A form test of revision equal to zero or being negative will be carried out in the Section \ref{NullTest}. 
