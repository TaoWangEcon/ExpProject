...
\begin{document}
	...
	Overal the results derived in coming sections are  not sensitive to if the shock is serially correlated. Also, it allows for the possibility of time varying volatility, i.e. the variance of $\omega$ is thus $\sigma^2_{t,\omega}$ with time subsript. 
	...
	It is easy to show that the average forecast errors are serially correlated(Equation \ref{FESEPop}). For checking, setting $\lambda=1$, then the SE collapses to FIRE, in which there is no serial correlation between forecast errors and it fully respond to newly realized shocks at time $t$.  
	...
	\end{eqnarray}
	 ...
	 Similarly, the inefficiency of reducing uncertainty in SE take the following form in aggregate level. Average uncertainty at any point of time is now a weighted average of uncertainty to agents whose last updates have taken place in different periods of past. 
	...
	\begin{itemize}
		...
		\item Population average uncertainty revision underreacts to volatility of the shocks.
	\end{itemize}
	...
	where the Kalman gain is a $P$ of size of two that determines the degrees of reaction to signals. 
	...
	Kalman filtering updates the variance according to normal updating.   The posterior variance at time $t$ is a linear function of uncertainty in previous period and variance of signals. 
    ...
    This directly gives the revision in uncertainty from time $t-1$ to $t$. The newly arrived information, although noisy, still brings aobut information gains, thus leading to an unambiguously drop on uncertainty. But due to the signal is not perfect, i.e. $\Sigma^v \neq 0$, there is inefficiency in reducing uncertainty compared to FIRE.
	...
	Figure \ref{IllustrateNI} illustrates the Kalman filtering problem over the $10$ period horizon from to $t$ to $t+10$  following one-time shock of one standard deviation $\omega_t$.  For illustration purpose, I assume that both private and public signals have the equal degree of noisiness and their standard deviation is equal to the unconditional standard deviation of $y$, i.e. $\frac{\sigma^2_\omega}{1-\rho}$.  A particular draw of public and private signals their respetive conditional distributions of given $y_t$ are plotted.    
	
	From the top left figure, the agent's nowcasting $y_{t+k|t+k}$, namely her best real-time guess based on prior and the realized signal at each period ranghly tracks the $y_{t+k}$ and responds partially to signals in each period. Correspondingly, her forecating of $y$ at $t+10$ also centers around the true value. 
	
	FIRE implies agent learns realized $y$ in each period perfectly. Thus the nowcasting remains zero thoughout the whole horizon. Nowcasting uncertainty picks up in the realization of shock and gradully declines as more and more informaiton signals are realized. This comes from the information gains from Kalman filtering. 
	
    The sharp rise in nowcasting uncertainty in the begining of the period is only humbly reflected in the forecasting uncertainty seems counter intuitive in the first sight. Actually this is due to nowcasing uncertainty is discounted by the factor of $\rho^2(h-k)$.  (Equation \ref{VarNIEq}). It is also interesing that for the particular configurations of noisiness of the signals, the drop in forecasting uncertainty from NI is not as inefficient as one may expected compared to FIRE.  This has important quantitative implications for NI models. In particular, the nosiness of the signals need to be consistent with the degree of rigidity. I will return this point in the next section.  
...
\end{itemize}
	...
	Taking stock, both SE and NI predict rigidity in expectation incorporating arrival of new information. In the former context, the rigidity comes from the simple non-updating of the most realization of the variable. In the later case, the rigidity comes from a partial reaction to new information due to its nosiness.  In addition to forecast and disagreements, the rigidity is also reflected in the dynamics of uncertainty.  
	
	According to FIRE, the revision in uncertainty from $t-1$ to $t$ is eactly equal to  the variance of the shock that is realized at time $t$. Taking $h=1$ as a simple and plug it in Equation \ref{VarREPop}, we have
	
	\begin{eqnarray}\label{VarREPopRv1}
	\bar \sigma^{*2}(y_{t+1}|t) - \bar \sigma^{*2}(y_{t+1}|t-1) = - \rho^2\sigma^2_\omega
	\end{eqnarray}

	In SE, the one-period revision in uncertainty is less due the non-npdate of population. 

	\begin{eqnarray}\label{VarSEPopRv1}
     \bar \sigma^{2}(y_{t+1}|t) - \bar \sigma^{2}(y_{t+1}|t-1) = (1-\lambda) - \rho^2\sigma^2_\omega
    \end{eqnarray}
	
	
	
		
	Figure \ref{ir_pop} illustrats the impulse response of population moments to a hypothetical one-time shock at time $t$ according to different theories.  
	
	on the dynamics of revision as one approaches the terminate date, i.e. forecast of $y_{t+h}$ at $t$, $t+1$, $t+2$ till $t+h$ instead of forecasts with fixed horizon, i.e. $y_{t+h}$,$y_{t+h+1}$. 
