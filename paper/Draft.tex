...
This section first reproduces the results of a list of statistical tests of FIRE used in the existing literature in Table \ref{NullTestTable}, primarily following \citet{mankiw2003disagreement},  and then extends the tests replying upon data of individual uncertainty in Table \ref{RevEfficency} and \ref{WeakRevEfficiency}, in the spirit of forecasting efficiency by \citet{nordhaus1987forecasting}. It can be seen as an extension of what \citet{fuhrer2018intrinsic} does to mean forecast to uncertainty.  
...
FE-based tests results are prensented in Table \ref{NullTestTable}. Individual level data are used thanks to the panel structure of both surveys. But since test 2 and 3 requires individual forecasts in vintages that are more than one year apart while SCE only surveys each household for 12 months, the two tests are done for only SPF forecasts of CPI and PCE. Also,the regresions are adjusted corresponding to the quarterly and monthly frequency of SPF and SCE. Since these regressions are based on 1-year inflations in overlapping periods, white standard error is computed for hypothesis testing. 
...
The second batch of tests in Table \ref{RevEfficency} focus on testing forecasting efficiency using revisions of mean forecasts and uncertainty, as referred as Revision-based test.  

In plain words, it checks if the revision from 1-year-ahead forecast to nowcast of current-year inflation is efficient by two criteria. (1) Forecast revision does not depend on past information, including the past revisions of forecast. (2) Drop in uncertainy are not serially correlated. 

 \citet{fuhrer2018intrinsic}'s test for mean forecast revision takes the following form(using 1 period revision as an exampl).

\begin{eqnarray}\label{RevEffMeanTest}
y_{i,t+1|t+1} - y_{i,t+1|t}=\alpha + \beta (y_{i,t+1|t} - y_{i,t+1|t-1})+\epsilon_{i,t+1}
\end{eqnarray}

$\beta =0$ according to FIRE, because rational forecast revision only responds to newly realized shocks thus it is note predictable by past revisions.\footnote{Adding $y_{t+1|t}$ to both sides of Equation \ref{RevEffMeanTest} gives an equivalent null hypothesis used by \citet{fuhrer2018instrinsic}: coefficient of regression of $y_{t+1|t+1}$ on  $y_{t+1|t}$ is $1-\beta=1$. } Since we have four vintages of the forecasts from SPF, we can check if the revision in nowcasting are serially correlated with 1-year-ahead forecats in previous three quarters. 
